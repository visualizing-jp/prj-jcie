...
class DiseaseApp {
  ...
  async init() {
    try {
      // data-disease 属性から disease ID を取得
      const diseaseId = document.body.dataset.disease;
      if (!diseaseId) {
        console.error('data-disease attribute not found on <body>');
        return;
      }
      ...
      console.error('DiseaseApp initialization failed:', error);
    }
  }
